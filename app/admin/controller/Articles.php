...
class Articles extends Common
{
    ...
    public function lists(){
        ...
        $catname=$catname?$catname:"PC中文内容管理";
    ...
    public function add(){
        $catids = getTree2('category',0,1,0);
    ...
    public function index2(){
        $category=db('category');
        $ismenu = "ismenu=1 or ismenu=2";
        $volist=$category->where($ismenu)->where('lang',2)->field("catname as name,id,parentid,child,module")->order('parentid asc,listorder asc')->select();
        $fhref="";
        $title="";
        foreach($volist as $key=>$value){
            if(!$value['child']){
                if($value['module']=='page'){
                    $url=url('page/edit', array('id'=>$value['id']));
                }elseif($value['module']=='enproduct'){
                    $url=url('enproduct/index', array('catid'=>$value['id']));
                }elseif($value['module']=='enpicture'){
                    $url=url('enpicture/index', array('catid'=>$value['id']));
                }else{
                    $url=url('Articles/lists2',['catid'=>$value['id']]);
                }
                if($fhref==""){
                    $fhref=$url;
                }
    ...
    public function lists2(){
        if(request()->isPost()) {
            ...
            $page =input('page')?input('page'):1;
            $pageSize =input('limit')?input('limit'):config('pageSize');
            $list = Db::table(config('database.prefix') . 'enarticle')->alias('a')
                ->join(config('database.prefix') . 'category at', 'a.catid = at.id', 'left')
                ->field('a.*,at.catname as typename')
                ->where('a.title', 'like', "%" . $key . "%")
                ->where($catwhere)
        ...
        $catname=$catname?$catname:"PC英文内容管理";
        $this->assign('mTitle',$catname);
        $this->assign('catid',$catid);
        return $this->fetch();
    }
    ...
    //禁用、开启
    public function editState2(){
        $id=input('post.id');
        $open=db('enarticle')->where(array('id'=>$id))->value('open');//判断当前状态情况
        if($open==1){
            $data['open'] = 0;
            db('enarticle')->where(array('id'=>$id))->setField($data);
            $data['status'] = 1;
        }else{
            $data['open'] = 1;
            db('enarticle')->where(array('id'=>$id))->setField($data);
